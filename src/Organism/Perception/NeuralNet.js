...
********************************************************************************/
    ...
    feedForward(sensoryData) {
        ...
        for (var conn of this.connections) {
            if (conn.sinkType == ACTION && !neuronOutputsComputed) {
                ...
                for (
                    let neuronIndex = 0;
                    neuronIndex < this.neurons.length;
                    ++neuronIndex
                ) {
                    if (this.neurons[neuronIndex].driven) {
                        this.neurons[neuronIndex].output = Math.tanh(
            ...
            if (conn.sinkType == ACTION) {
                ...
                neuronAccumulators[conn.sinkNum] +=
                    inputVal * conn.weightAsFloat();
            }
        }
    ...
    makeNodeList(connectionList) {
        ...
        for (var conn of connectionList) {
            if (conn.sinkType == NEURON) {
                let selfInput =
                    conn.sourceType == NEURON && conn.sourceNum == conn.sinkNum;
        ...
        while (!allDone) {
            ...
            for (var [number, neuron] of nodeMap.entries()) {
                ...
                if (neuron.numOutputs == neuron.numSelfInputs) {
                    ...
                    this.removeConnectionsToNeuron(
                        connectionList,
                        nodeMap,
                        number
                    );
